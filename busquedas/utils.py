...
class FIFOQueue(Queue):
    ...
    def pop(self):
        """"return self.A.pop(0) toda la lista se desplaza, lo de abajo para que se mueva cada cierto nivel"""
        e = self.A[self.start]
        self.start += 1
        if self.start > 5 and self.start > len(self.A) / 2:
            ...
            self.start = 0
        return e
"""Reordenar la lista abierta cuando haces extend
el menor valor al final? al final.
Reordenar es algo común, los objetos de esa lista son objetos. Ordenamos con cual criterio?
"""


def returnPathCost(nodo):
    return nodo.path_cost


def pathcostplush(nodo, problem):
    sum = nodo.path_cost + problem.h(nodo)
    return sum
...
class babg(Queue):
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A.sort(key=returnPathCost)
...
#Tiene que tener acceso a la instancia del problema, para acceder a h.
